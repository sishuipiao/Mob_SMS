...
- (void)drawRect:(CGRect)rect 
{
	...
	CGSize numberSize = [self.value sizeWithAttributes:attributes];
...
*/	
	...
	[self.value drawAtPoint:textPt withAttributes:attributes];
...
@implementation UIView(HYZBadgeView)

- (HYZBadgeView *)badge
{
    id exist = [self viewWithTag:HYZ_BADGE_TAG];
    if(exist) {
        if(![exist isKindOfClass:[HYZBadgeView class]]) {
            //NSLog(@"Unexpected view of class %@ found with badge tag.",existingView);
            return nil;
        } else {
            return (HYZBadgeView *)exist;
        }
    }
    HYZBadgeView *badge = [[HYZBadgeView alloc]initWithFrame:CGRectZero];
